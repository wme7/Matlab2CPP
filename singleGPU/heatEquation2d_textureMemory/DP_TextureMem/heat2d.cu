...
void Call_Laplace(dim3 numBlocks, dim3 threadsPerBlock, double *d_u, double *d_un) 
{
  ...
  // bind textures
  // cudaBindTexture2D(0,&tex_u, &d_u[0], &desc,NX,NY,NX*sizeof(float));
  // cudaBindTexture2D(0,&tex_un,&d_un[0],&desc,NX,NY,NX*sizeof(float));
  cudaBindTexture2D(0,&tex_u, &d_u[0], &desc,NX,NY,NX*sizeof(double));
  cudaBindTexture2D(0,&tex_un,&d_un[0],&desc,NX,NY,NX*sizeof(double));
