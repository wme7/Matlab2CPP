...
template <typename T> __device__ void AtomicMax(T * const address, const T value)
{
 ...
 */
template <> __device__ void AtomicMax(float * const address, const float value)
{
	if (* address >= value)
	{
		return;
 ...
 * @param[in]	value	The value that is compared to the reference in order to determine the maximum
 */
template <> __device__ void AtomicMax(double * const address, const double value)
{
	if (* address >= value)
	{
		return;
 ...
 */
extern "C" void CopyDevHalosToBlock(real * devBlock, const real * devHaloLineLeft, const real * devHaloLineRight,const int2 * size, const int * neighbors)
{
