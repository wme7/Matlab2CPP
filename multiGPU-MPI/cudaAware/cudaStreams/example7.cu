...
int main() {
    ...
    for (int k = 0; k < numGPUs; k++){
        ...
        cudaMalloc(&(plan[k].d_elements), NperGPU*sizeof(double));
    ...
    // --- "Breadth-first" approach - async copy with streams
    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpyAsync(plan[k].d_elements, plan[k].h_elements, NperGPU*sizeof(double), cudaMemcpyHostToDevice, plan[k].stream));
    }

    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        kernelFunction<<<iDivUp(NperGPU, BLOCKSIZE), BLOCKSIZE, 0, plan[k].stream>>>(plan[k].d_elements, NperGPU);
    }

    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpyAsync(plan[k].h_elements, plan[k].d_elements, NperGPU*sizeof(double), cudaMemcpyDeviceToHost, plan[k].stream));
    }
