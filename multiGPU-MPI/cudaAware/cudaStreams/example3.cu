...
int main() {
    ...
    for (int k = 0; k < numGPUs; k++){
        ...
        gpuErrchk(cudaMalloc(&(plan[k].elements), NperGPU*sizeof(double)));
    }
    ...
    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpy(plan[k].elements, inputMatrix+k*NperGPU, NperGPU*sizeof(double), cudaMemcpyHostToDevice));
    }

    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        kernelFunction<<<iDivUp(NperGPU, BLOCKSIZE), BLOCKSIZE>>>(plan[k].elements, NperGPU);
    }

    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpy(inputMatrix+k*NperGPU, plan[k].elements, NperGPU*sizeof(double), cudaMemcpyDeviceToHost));
    }
