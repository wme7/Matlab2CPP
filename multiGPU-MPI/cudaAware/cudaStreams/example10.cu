...
int main() {
    ...
    double *inputMatrix; 
    gpuErrchk(cudaMallocHost(&inputMatrix,N*sizeof(double)));

    // --- "Breadth-first" approach - async copy with deafult stream and common host buffer
    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpyAsync(plan[k].d_elements, inputMatrix+k*NperGPU, NperGPU*sizeof(double), cudaMemcpyHostToDevice));
    }
    ...
    for (int k = 0; k < numGPUs; k++) {
        gpuErrchk(cudaSetDevice(k));
        gpuErrchk(cudaMemcpyAsync(inputMatrix+k*NperGPU, plan[k].d_elements, NperGPU*sizeof(double), cudaMemcpyDeviceToHost));
    }
