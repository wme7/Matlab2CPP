...
int main(int argc, char** argv)
{
  ...
  h_un = (REAL*)malloc(sizeof(REAL)*Nx*Ny);

  // Set Domain Initial Condition and BCs
  Call_Init(4,h_u,dx,dy,Nx,Ny);
  ...
  switch (UseSolverNo) {
    case 1: {
      ...
      Call_CPU_Jacobi2d_v2(h_u,h_un,max_iters,kx,ky,Nx,Ny);
      break;
    }
    case 2: {
      printf("Using CPU solver with Neumann BCs \n");
      Call_CPU_Jacobi2d(h_u,h_un,max_iters,kx,ky,Nx,Ny);
      break;
    }
    case 3: {
      printf("Using OMP solver with Dirichlet BCs \n");
      Call_OMP_Jacobi2d_v2(h_u,h_un,max_iters,kx,ky,Nx,Ny);
      break;
    }
    case 4: {
      printf("Using OMP solver with Neumann BCs \n");
      Call_OMP_Jacobi2d(h_u,h_un,max_iters,kx,ky,Nx,Ny);
      break;
    }
  }
  ...
  if (DEBUG) print2D(h_un,Nx,Ny);

  float gflops = CalcGflops(tCPU, max_iters, Nx, Ny);
  PrintSummary("HeatEq2D (7-pt)", "none", tCPU, gflops, tFinal, max_iters, Nx, Ny);
  CalcError(h_un, tFinal, dx, dy, Nx, Ny);

  // Write solution to file
  if (WRITE) Save2D(h_un,Nx,Ny); 
